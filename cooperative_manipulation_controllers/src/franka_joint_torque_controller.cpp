...
void JointImpedanceExampleController::update(const ros::Time& /*time*/,
  ...
  if (vel_current_ < vel_y) {
    // Acceleration
    vel_current_ += period.toSec() * std::fabs(vel_acc_/10);
    vel_current_ = std::fmin(vel_current_, vel_y);
  }
  else if (vel_current_ > vel_y) {
    // Deceleration
    vel_current_ -= period.toSec() * std::fabs(vel_acc_/100);
    vel_current_ = std::fmax(vel_current_, vel_y);
  }
  else if(vel_y == 0.0){
    // Stop
    if (vel_current_ != 0.0){
...
//     }
//   }
//   sub_pose_command_ = node_handle.subscribe("vel_trans_command", 1, &JointImpedanceExampleController::velocityCmdCallback,this,ros::TransportHints().reliable().tcpNoDelay());
...
// void JointImpedanceExampleController::velocityCmdCallback(const std_msgs::Float64MultiArray::ConstPtr& tau_cmd)
// {
//   if (tau_cmd->data.size() != 7) {
//     ROS_ERROR_STREAM(
//           "EffortJointTorqueController: Published Commands are not of size 7");
//   }
//   else{
//     for (size_t i = 0; i < 7; ++i) {
//     vel_trans_command[i] = tau_cmd->data[i];
...
//   // std::array<double, 16> pose_desired = cartesian_pose_handle_->getRobotState().O_T_EE;
//   // pose_desired[13] += 0.0;
//   // pose_desired[14] += 0.0;
//   cartesian_pose_handle_->setCommand(pose_desired);
  
//   // ROS_INFO_STREAM("pose_desired[13]");
//   // ROS_INFO_STREAM(pose_desired[13]);
...
//   std::array<double, 7> tau_d_calculated;
//   for (size_t i = 0; i < 7; ++i) {
//     tau_d_calculated[i] = coriolis_factor_ * coriolis[i] + pose_command[i];
//   }
