...
//-------------------------------------------------------------------------------------------------------
  sub_tau_command_ = node_handle.subscribe("/franka_joint_torque_controller/command", 1, &FrankaJointTorqueController::chatterCallback,this,ros::TransportHints().reliable().tcpNoDelay());
...
void FrankaJointTorqueController::update(const ros::Time& /*time*/, const ros::Duration& period) {
  ...
  for (size_t i = 0; i < 7; ++i) {
    ...
    ROS_INFO("I heard: [%f]", tau_command[i]);
    }
  ...
  for (size_t i = 0; i < 7; ++i) {
    last_tau_d_[i] = tau_d_saturated[i];
  }
}
