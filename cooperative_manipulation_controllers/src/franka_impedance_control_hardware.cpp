...
bool FrankaImpedanceController::init(hardware_interface::RobotHW* robot_hw,
                                           ros::NodeHandle& node_handle) {
                                            ...
                                            ROS_INFO_STREAM("testaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
  std::string arm_id;
...
void FrankaImpedanceController::starting(const ros::Time& /*time*/) {
...
void FrankaImpedanceController::update(const ros::Time& /*time*/,
  ...
  // pose_error[0] = desired_position[0] - current_position[0];
  // pose_error[1] = desired_position[1] - current_position[1];
  // pose_error[2] = desired_position[2] - current_position[2];

  // Orientation pose_error
  // Eigen::Matrix<double, 3, 1> orientation_error_matrix;
 ...
 Eigen::VectorXd tau_task(7), tau_nullspace(7), tau_d(7);
  ...
  tau_task << jacobian.transpose() *
                  (-stiffness * pose_error - damping * (jacobian * dq));
...
Eigen::Matrix<double, 7, 1> FrankaImpedanceController::saturateTorqueRate(
